...
router.post('/login', (req, res) => {
  ...
  res.json({
    token,
    user: { id: user.id, username: user.username, fullName: user.full_name, roleId: user.role_id, roleName: user.role_name, staffId: user.staff_id ?? null, staffName: user.staff_name ?? null, isActive: true, mustChangePassword: user.must_change_password === 1 },
    permissions: getEffectivePermissions(user.id),
  });
});
router.get('/me', requireAuth, (req, res) => {
  const user = getDb().prepare('SELECT u.id, u.username, u.full_name fullName, u.role_id roleId, r.name roleName, u.staff_id staffId, s.full_name staffName, u.is_active isActive, u.must_change_password mustChangePassword FROM users u JOIN roles r ON r.id = u.role_id LEFT JOIN staff s ON s.id = u.staff_id WHERE u.id = ?').get(req.user!.id) as Record<string, unknown> | undefined;
  if (user) user.mustChangePassword = user.mustChangePassword === 1;
