...
router.post('/login', (req, res) => {
  ...
  const user = getDb().prepare('SELECT u.*, r.name role_name FROM users u JOIN roles r ON r.id = u.role_id WHERE username = ? AND u.is_active = 1').get(username) as { id: number; username: string; full_name: string; role_id: number; role_name: string; password_hash: string; staff_id: number | null } | undefined;
  ...
  res.json({ token, user: { id: user.id, username: user.username, fullName: user.full_name, roleId: user.role_id, roleName: user.role_name, staffId: user.staff_id ?? null, isActive: true } });
});
