...
import { recordAcknowledgement } from '../services/signatureService.js';
...
<div class="footer"><span>SECH_LIMS by Nickland — Attestation List</span><span>Personally signed by each staff member; signatures are non-transferable.</span></div>
  ...
  router.post('/:id/attest', requirePermission('documents.library', 'view'), (req, res) => {
    ...
    try { recordAcknowledgement(req, { moduleKey: 'documents', recordType: 'document_attestations', recordId: attestation.id, purpose: 'document_acknowledgement', meaning: 'Acknowledged controlled document' }); } catch { /* trail is best-effort */ }
