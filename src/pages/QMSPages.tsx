...
export function RisksPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  return <div>
    ...
    {tab === 'New Risk' && <div className="card">{can('risks', 'create') && <form className="form" onSubmit={submit}>
      ...
      <label>Risk area<TextField value={formState.riskArea} onValue={nextValue => setFormState(prev => ({ ...prev, riskArea: nextValue }))} placeholder="Describe the risk area" required /></label>
      <label>Risk description<TextField as="textarea" value={formState.riskDescription} onValue={nextValue => setFormState(prev => ({ ...prev, riskDescription: nextValue }))} placeholder="Describe the risk" required /></label>
      <label>Cause<TextField as="textarea" value={formState.cause} onValue={nextValue => setFormState(prev => ({ ...prev, cause: nextValue }))} placeholder="Root cause or source" /></label>
      <label>Consequence<TextField as="textarea" value={formState.consequence} onValue={nextValue => setFormState(prev => ({ ...prev, consequence: nextValue }))} placeholder="Potential consequence" /></label>
      <label>Existing controls<TextField as="textarea" value={formState.existingControls} onValue={nextValue => setFormState(prev => ({ ...prev, existingControls: nextValue }))} placeholder="Existing control measures" /></label>
      ...
      <label>Mitigation plan<TextField as="textarea" value={formState.mitigationPlan} onValue={nextValue => setFormState(prev => ({ ...prev, mitigationPlan: nextValue }))} placeholder="Planned mitigation" /></label>
    ...
    {selected && <div className="card"><h3>Risk detail</h3>
      ...
      <div className="form" style={{ display: 'grid', gap: '14px', marginTop: '14px' }}>
        <div><h4>Review risk</h4><label>Notes<TextField as="textarea" value={workflowState.reviewNotes} onValue={nextValue => setWorkflowState(prev => ({ ...prev, reviewNotes: nextValue }))} placeholder="Enter review notes" /></label><label>Next review date<input type="date" value={workflowState.nextReviewDate} onChange={e => setWorkflowState(prev => ({ ...prev, nextReviewDate: e.target.value }))} /></label>{can('risks', 'approve') && <button onClick={() => reviewRisk(selected.id)}>Submit review</button>}</div>
        <div><h4>Action from risk</h4><label>Title<TextField value={workflowState.actionTitle} onValue={nextValue => setWorkflowState(prev => ({ ...prev, actionTitle: nextValue }))} placeholder="Action title" /></label><label>Description<TextField as="textarea" value={workflowState.actionDescription} onValue={nextValue => setWorkflowState(prev => ({ ...prev, actionDescription: nextValue }))} placeholder="Enter action description" /></label><label>Assigned to<select value={workflowState.actionAssignedToStaffId} onChange={e => setWorkflowState(prev => ({ ...prev, actionAssignedToStaffId: e.target.value }))}><option value="">Select staff</option>{staff.map(s => <option key={s.id} value={s.id}>{s.fullName}</option>)}</select></label><label>Due date<input type="date" value={workflowState.actionDueDate} onChange={e => setWorkflowState(prev => ({ ...prev, actionDueDate: e.target.value }))} /></label><label>Priority<select value={workflowState.actionPriority} onChange={e => setWorkflowState(prev => ({ ...prev, actionPriority: e.target.value }))}><option value="normal">Normal</option><option value="high">High</option><option value="low">Low</option></select></label>{can('actions', 'create') && <button onClick={() => createActionFromRisk(selected.id)}>Create action</button>}</div>
        <div><h4>Link CAPA</h4>{can('nc_capa', 'create') && <button onClick={() => createCapaFromRisk(selected.id)}>Create CAPA</button>}<button className="secondary" onClick={() => closeRisk(selected.id)}>Close risk</button></div>
      </div>
    </div>}
  </div>;
}
